...
const handleViewPrizeDraw = (item, i) => {
    ...
    game.participant[2].available = false
    localStorage.setItem("draw_game", JSON.stringify(game))
...
const createSectionDraw = () => {
    ...
    contentDetail.className = "prize-detail-container"
    playArea.className = "play-area"
    title.className = "title-section"
    
    contentContainer.append(contentDetail)
...
const createSectionPrizes = () => {
    ...
    const createList = (item, i) => {
        ...
        if (!(item?.winners?.length > 0)) left = item?.total - item?.winners?.length
...
const createSectionPlayers = () => {
    ...
    const createList = (item) => {
        ...
        prize.textContent = `(Sepeda Motor)`
        tag.textContent = "❚ "
