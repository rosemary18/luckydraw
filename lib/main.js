...
const handlerFallPlayer = (index, draw = true) => {
    
    if (typeof index == "number") {
        game?.participant?.forEach((item, i) => {
            if (item?.id == stageWinner?.[index]?.id) {
                if (definedWinner?.length > 0) definedWinner.forEach((winner, j) => winner?.id == item?.id && definedWinner?.splice(j, 1))
                game.participant[i].available = false
            }
        })
    } else {
        definedWinner = []
    ...
    if (typeof index == "number" && draw) localStorage.setItem('stage_winner', `draw-${index+1}`)
    if (!draw) localStorage.removeItem('stage_winner')
