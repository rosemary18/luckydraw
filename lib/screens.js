                ...
                <div class="flex bg-[#374251] flex-col rounded w-[23em] shadow-[0_2px_50px_-10px_rgba(33,150,243,.3)] p-[2px]">
                    ...
                    <div class="flex flex-col bg-[#101827] rounded p-3">
                        <div class="flex flex-col">
                            <label class="block mb-2 text-xs font-medium text-gray-900 dark:text-white" for="file_input">Upload file hadiah</label>
                            <input class="block w-full p-1 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" aria-describedby="file_input_prizes" id="file_prizes" type="file" accept=".txt">
                        </div>
                        <div class="flex flex-col mt-2">
                            <label class="block mb-2 text-xs font-medium text-gray-900 dark:text-white" for="file_input">Upload file peserta</label>
                            <input class="block w-full p-1 text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" aria-describedby="file_input_participants" id="file_participants" type="file" accept=".txt">
                        </div>
...
const DashboardScreen = () => {
    ...
    screen.selectPrize = async (id) => {
        ...
        if (!STATES.SPIN_SCREEN) {
            ...
            setTimeout(() => window.open('./spin.html', 'tab-spin'), 100)
        }
    ...
    screen.draw = () => {
        ...
        if (slots?.length > 0) {
            ...
            if (available_participants.length < slots.filter?.(slot => slot?.checked).length) return alert(`Peserta tidak cukup untuk melakukan pengundian!`)
    ...
    screen.redraw = (index) => {
        ...
        if (index) {
            ...
            if (available_participants.length < (STATES.ACTIVE_PRIZE?.total - STATES.ACTIVE_PRIZE?.winners.length)) return alert(`Peserta tidak cukup untuk pengundian selanjutnya!`)
            if (STATES.STAGE_WINNER?.some(w => w?.spin == true)) return alert(`Masih ada undian yang belum selesai!`)
            else {
                const stage_winners = []
                const slots = document.getElementsByName('slot-draw')
    ...
    screen.saveDefineWinners = () => {
        ...
        const defined = []
        const definedWinners = document.getElementsByName('defined-winner')

        if (definedWinners?.length > 0) {

            definedWinners.forEach(w => {
                if (w?.value == "-") defined.push(null)
                else {
                    STATES.GAME.participants.forEach((p, i) => {
                        if (p?.id == w.value) {
    ...
    screen.render(({ checkParticipant }) => `
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center justify-start">
                        <a class="flex items-center">
                            ...
                            <span class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Lucky Draw</span>
                        </a>
                    </div>
                        ...
                        <div class="flex items-center ml-3">
                            <div class="hidden w-full md:block md:w-auto mr-2" id="navbar-solid-bg">
                                <ul class="flex flex-col font-medium mt-4 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:bg-transparent dark:bg-gray-800 md:dark:bg-transparent dark:border-gray-700">
                                    <li>
                                        <a class="block py-2 pl-3 pr-4 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent" onclick="${f}export()">Export</a>
                                    </li>
                                    <li>
                                        <a class="block py-2 pl-3 pr-4 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent" onclick="${f}close()">Keluar</a>
                                    </li>
                                </ul>
                            </div>
                        </div>
        ...
        <aside class="fixed top-0 left-0 z-40 w-64 h-screen pt-[4.5em] transition-transform -translate-x-full bg-white border-r border-gray-200 sm:translate-x-0 dark:bg-gray-800 dark:border-gray-700" aria-label="Sidebar">
            <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-gray-800">
                <ul class="space-y-2 font-medium">
                    ${
                        STATES.GAME?.prizes?.map((item) => `
                            <li class="cursor-pointer">
                                <a class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white ${STATES.ACTIVE_PRIZE?.id == item?.id ? 'bg-[#2196f3]' : 'hover:bg-gray-700'} group" onclick="${f}selectPrize('${item?.id}')">
                                    <span class="flex-1 ml-1 whitespace-nowrap">${item?.prize || "-"}</span>
                                    ${
                                        (item?.total - item?.winners?.length) > 0 ? `<span class="inline-flex items-center justify-center w-3 h-3 p-3 ml-3 text-sm font-medium text-gray-700 bg-gray-300 rounded-full dark:bg-gray-700 dark:text-gray-300">${(item?.total - item?.winners?.length)}</span>` 
        ...
        <div class="p-4 sm:ml-64 flex pt-[4.6em] h-full">
            <div class="flex flex-[2] flex-col mr-4 rounded-lg">
                <div class="flex flex-col overflow-y-auto flex-1 rounded-lg bg-gray-50 dark:bg-gray-800 p-4">
                    ${
                ...
                ${
                    (Object.keys(STATES.ACTIVE_PRIZE).length > 0 && (STATES.ACTIVE_PRIZE?.total - STATES.ACTIVE_PRIZE?.winners?.length) > 0) ? `
                        <div class="flex h-[16em] rounded-lg bg-gray-50 dark:bg-gray-800 mt-4 p-4">
                            <div class="flex flex-col flex-1">
                                ${
                                    Array(Math.max(0, Math.min(((STATES.ACTIVE_PRIZE?.total-STATES.ACTIVE_PRIZE?.winners.length) || 0), 5))).fill({}).map((_, i) => `
                                        <div id="con-draw-${i}" class="flex items-center my-2">
                                            <input id="draw-${i}" name="slot-draw" ${(i == 0) ? 'disabled' : ''} type="checkbox" checked onchange="${f}onActiveDraw('${i}')" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600 mr-4">
            ...
            <div class="flex flex-1 max-h-[100%] overflow-y-auto p-4 flex-col border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
            ...
            <div class="flex flex-1 flex-col items-end h-full max-h-full">
                <div class="flex flex-col h-full max-h-full rounded-l-md shadow bg-gray-800 w-[24em]">
                    <div class="flex items-center px-4 py-2">                    
                        <h3 class="text-lg font-medium text-gray-900 dark:text-white">Definisikan Pemenang</h3>
                        <button onclick="${f}closeModalDefineWinner()" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ml-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
